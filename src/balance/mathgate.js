 ...
 * 이제는 세 장 중에서 고른다 — 난이도 보정을 **설계자가 아니라 푸는 사람이** 한다.
 * "이거 너무 쉬운데?" 싶으면 빨간 카드를 집어 더 받아 가면 된다.
 *
 * 세 장은 항상 서로 다른 난이도여야 한다(같은 걸 두 장 내밀면 선택이 아니다).
...
export const CARD_STYLE = [
  { key: 'safe',  emoji: '🟢', name: '안전', note: '차분하게 확실히' },
  { key: 'std',   emoji: '🔵', name: '정석', note: '이 조합에 딱 맞게' },
  { key: 'bold',  emoji: '🔴', name: '도전', note: '어려운 만큼 크게' },
];
/* base보다 어려운 걸 고를수록 환급이 커진다 — 어려운 카드를 집을 이유.
...
/* 별조각은 "센 관문에서 더 센 걸 골라 한 번에 통과"했을 때만 준다.
 * 흔해지면 영구 축복의 의미가 사라지므로 조건을 일부러 좁게 잡았다. */
export const cardShards = (lv, base) => (clampLv(lv) > base && clampLv(lv) >= 4 ? 1 : 0);

/* ---------- 적응형 보정 ----------
 * 카드로 스스로 고르게 해도 늘 같은 자리에 머무는 사람이 있다 —
 * 쉬운 카드만 집어 술술 넘기거나, 어려운 카드만 집어 계속 막히거나.
 * 그래서 최근 성적을 보고 카드 **세 장을 통째로** 한 칸 밀어 준다.
 * 카드 선택은 그대로 살아 있고, 고를 수 있는 범위만 움직인다.
 *
