 ...
 * 이제는 세 장이 펼쳐지고 **룰렛이 돌아 하나가 뽑힌다.** 고르지 않는다.
 *
 * ▸ 왜 고르게 하지 않는가
 *   한때는 골라서 풀게 했다. 난이도 보정을 푸는 사람 손에 넘긴다는 뜻이었는데,
 *   실제로 해 보니 **조합할 때마다 메뉴를 하나 더 넘기는 일**이 됐다.
 *   조합은 연쇄로 이어지므로 그 결정이 한 판에 열댓 번 반복된다 — 재미가 아니라 짐이다.
 *   난이도가 어긋나던 진짜 원인(하한선 없음 · lv이 수의 크기를 안 건드림)은 이미
 *   생성기 쪽에서 고쳤고, 사람마다 다른 편차는 적응형 보정이 알아서 맞춘다.
 *   그래서 선택은 지우고 **변덕**만 남겼다: 뭐가 걸릴지 모르는 한 번의 뽑기.
 *   결정 비용은 0이 되고 긴장은 남는다.
 *
 * 세 장은 항상 서로 다른 난이도여야 한다(같은 게 두 장이면 뽑기가 심심하다).
 ...
 * 매번 걸리면 벌칙이 된다. 그래서 관문이 셀수록 🔴가 귀해진다.
 * (초반 관문은 🔴가 걸려도 부담이 적으니 넉넉하게 준다 — 뽑는 맛이 거기서 산다) */
...
/* 어느 장이 걸렸는가. rnd는 주입받는다 — 밸런스 봇이 시드 난수로 같은 규칙을 밟아야 한다 */
...
/* base보다 어려운 게 걸릴수록 환급이 커진다 — 센 문제가 걸린 게 반가운 이유.
...
/* 별조각은 "센 관문에서 더 센 게 걸렸는데 한 번에 통과"했을 때만 준다.
 ...
 * 최근 성적을 보고 룰렛에 올라가는 **세 장을 통째로** 한 칸 밀어 준다.
 * 뽑기의 변덕은 그대로 두고, 뽑히는 범위만 조용히 움직인다.
 *
