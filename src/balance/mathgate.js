 ...
 *      순한 문제가 나올 때까지 껐다 켜면 되니 변덕이 아무 의미가 없어진다.
 *   ② 틀려도 새 문제가 무한히 나온다 → 정확도는 환급에만 영향을 주고,
 *      조합 성공 여부에는 영향이 없다. 결국 "아무렇게나 눌러도 언젠간 된다".
 *
 *   그래서 대가를 붙인다. 골드 벌금이 아니라 **그 조합 자체를 잠근다**:
 *   포기하거나 세 번 틀리면 그 조합은 이번 준비 단계 동안 못 한다(웨이브를 치르면 풀린다).
 *   조합이 이 게임의 유일한 성장 수단이므로, 이게 가장 자연스럽고 가장 아픈 대가다.
 *
 *   골드를 깎지 않는 이유: 깎으면 "정답을 맞혔는데 골드가 모자라요"라는 최악의 결말이
 *   생긴다. 힌트 구매에서도 같은 이유로 그 상황을 미리 막고 있다. */
export const MATH_TRIES = 3;        // 한 관문에서 틀릴 수 있는 횟수 (넘으면 잠긴다)
