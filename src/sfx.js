...
export const SFX = {
  ...
  feast()      { /* 잔치 팡파르 + 왁자지껄 —
    * 예전엔 0.7초짜리 팡파르 하나였다. 화면에서는 5초 동안 잔치가 벌어지는데
    * 소리만 먼저 끝나 버려서, 뒤쪽이 음소거된 파티처럼 보였다.
    * 그래서 팡파르 → 나팔 → 환호 → 박수 순으로 잔치 길이만큼 깔아 준다. */
    ...
    /* 종이 나팔 — 음이 위로 미끄러지는 게 나팔 소리의 전부다 */
    tone(330, 0.55, 0.34, 'sawtooth', 0.05, 990);
    tone(392, 1.5, 0.34, 'sawtooth', 0.045, 1180);
    SFX.cheer(0.75);
    SFX.cheer(2.3);
    SFX.clap(1.1);
    SFX.clap(3.0); },

  /* 사람 소리를 흉내 낼 순 없으니 "우와~"의 모양만 만든다 —
   * 넓은 밴드패스 잡음이 부풀었다 잦아들고, 그 위에 목소리 높이의 화음이 뜬다. */
  cheer(at = 0) {
    /* noise() 는 넣자마자 잦아든다 — 밴드를 셋으로 어긋나게 겹쳐 부풀어 오르는 모양을 만든다 */
    noise(at, 0.85, 0.04, 620, 0.3);
    noise(at + 0.08, 0.7, 0.05, 1050, 0.4);
    noise(at + 0.2, 0.55, 0.03, 1600, 0.5);
    flowTone([523, 587, 659], at + 0.05, 0.55, 'triangle', 0.028);
    flowTone([784, 880], at + 0.12, 0.45, 'sine', 0.022); },

  /* 박수 — 짧고 마른 잡음을 어긋난 간격으로 흩는다 (같은 간격이면 기계가 된다) */
  clap(at = 0) {
    const gaps = [0, 0.07, 0.13, 0.21, 0.26, 0.34, 0.41, 0.5];
    for (const g of gaps) noise(at + g, 0.045, 0.035, 2400, 0.7); },
};
